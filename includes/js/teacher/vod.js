...
$(document).on('change', '#video_link_type', function () {
	var video_type = $(this).val();
...
$('#vod_text_no_of_videos_included,#vod_text_duration,#vod_text_monthly_cost').on('input',function(){
	vod_text_no_of_videos_included = $('#vod_text_no_of_videos_included').val()??'0';
	vod_text_duration = $('#vod_text_duration').val()??'0';
	vod_text_monthly_cost = $('#vod_text_monthly_cost').val()??'0';
	subscription_type = $('#subscription_type').val()??'0';

	if (subscription_type == 'Package') {
		$('#vod_text_title').val('Video Pack ($'+vod_text_monthly_cost+' for '+vod_text_no_of_videos_included+' videos)');
	}else{
		$('#vod_text_title').val('Membership ($'+vod_text_monthly_cost+' per '+vod_text_duration+' days)');
...
$('#form-video-add').validate({
		...
		rules: {
				...
				embeded_code : {
					required:true,
					required:function(){
						if($('#video_link_type').val() == 'Youtube'){   
							return true;
			            }else if($('#video_link_type').val() == 'Vimeo'){   
							return true;
			            }else{
							return false;
						}
...
$(document).on('click', '.delete-video', function () {
		...
		.then((willProceed) => {
			if (willProceed) {
				$('#loader-div').show();
				$.ajax({
					url: baseURL+'/teacher-video-on-demand/delete',
					type: 'POST',
		...
		if (selectedIds.length == 0) {
			...
			.then((willProceed) => {
				if (willProceed) {
					$('#loader-div').show();
					$.ajax({
						url: baseURL+'/teacher-video-on-demand/delete-selected',
						type: 'POST',
...
maxLimit = 50;
 ...
 $(document).on('click', '.delete-video-subscriptions', function () {
		
		var subscriptionId = $(this).attr('data-id');
		swal({
		  title: 'Delete Video?',
    	  text: 'Are you sure you want to delete this subscriptin?',
		  icon: "warning",
		  buttons: true,
		  dangerMode: true,
		})
		.then((willProceed) => {
			if (willProceed) {
				$('#loader-div').show();
				$.ajax({
					url: baseURL+'/teacher-video-on-demand/subscriptions/delete',
					type: 'POST',
 ...
 $('#form-add-subscription').validate({
		errorElement: 'span', //default input error message container
		focusInvalid: false, // do not focus the last invalid input
		ignore: [],
		invalidHandler: function(form, validator) {
	    ...
	    messages : {
				...
				no_of_videos_included : {
					required : 'Please enter no. of videos.',
				},
				duration : {
					required : 'Please enter duration.',
				},
				monthly_cost : {
					required : 'Please enter monthly cost.',
				},
				status : {
 ...
 $(document).on('click', '.total_purchased_subscriptions', function () {
		var videoHTML = $(this).data('encode-html');
		$('.video-purchased-student-popup').modal('show');
	      $('#purchased-child-ips').html('');
	      $('#purchased-child-ips').append(JSON.parse(videoHTML));
});